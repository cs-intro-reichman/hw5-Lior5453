public class Wordle 
{
    // Temp function to count the amount of words in the file
    public static int countWords(String filename)
    {
        ...
        while (!in.isEmpty())
        {
            in.readString(); // skipping line each time
            count++;
        }
    ...
    {
        In in = new In(filename);
        String[] stringsArray = new String[countWords(filename)]; // creating string array in the length of the words in the file
        int index = 0;

        while(!in.isEmpty())
        {
            stringsArray[index] = in.readString();  
            index++;
        }
    ...
    public static void main(String[] args) 
    {
        ...
        while (attempt < MAX_ATTEMPTS && !won) 
        {
            ...
            guess = guess.toUpperCase(); // changing guess to uppercase to be same as secret word
